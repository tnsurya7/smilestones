...
export default function LanguageDevelopmentPage() {
  ...
  const loadAgeData = async (age: string) => {
    try {
      // Load existing language development data for this specific age
      const response = await fetch(`/api/language-development?child_id=${childId}&age=${age}`);
      if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const response = await fetch('/api/language-development', {
        method: 'POST',
      ...
      if (response.ok) {
        setToast({ message: 'Language Development saved successfully!', type: 'success' });
      } else {
        throw new Error('Failed to save');
      }
    } catch (error) {
      console.error('Error saving language development:', error);
      setToast({ message: 'Failed to save Language Development. Please try again.', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(`Language Development for ${selectedAge} months`, 15, yPos);
    yPos += 8;
    ...
    if (skills.length === 0) {
      doc.text('No language development skills found for this age.', 15, yPos);
    } else {
      skills.forEach((skill, index) => {
        if (yPos > pageHeight - 40) {
          ...
          doc.setFontSize(12);
          doc.setFont('helvetica', 'bold');
          doc.text('LANGUAGE DEVELOPMENT ASSESSMENT (Continued)', pageWidth / 2, 20, { align: 'center' });
          yPos = 30;
          doc.setFontSize(9);
          doc.setFont('helvetica', 'normal');
        }
    ...
    <div className="admin-dashboard min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Language Development Assessment</h1>
        ...
        {selectedAge && currentSkills.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6">
              Language Development for {selectedAge} months
            </h2>
        ...
        {!selectedAge && (
          <div className="bg-white rounded-xl shadow-lg p-8 sm:p-12 text-center">
            <p className="text-gray-500 text-base sm:text-lg italic">
              Please select child's age to view relevant language development
            </p>
          </div>
        )}
        ...
        {selectedAge && (
          <div className="flex justify-center pb-6 mt-6">
            ...
            >
              ...
              {saving ? 'Saving...' : 'Save Language Development'}
            </button>
          </div>
        )}
      </div>
