...
import { GROSS_MOTOR_SKILLS, GROSS_MOTOR_AVAILABLE_AGES } from '@/data/grossMotorSkills';
...
export default function GrossMotorSkillsPage() {
  ...
  const loadAgeData = async (age: string) => {
    try {
      // Load existing gross motor skills data for this specific age
      const response = await fetch(`/api/gross-motor-skills?child_id=${childId}&age=${age}`);
      if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const response = await fetch('/api/gross-motor-skills', {
        method: 'POST',
      ...
      if (response.ok) {
        setToast({ message: 'Gross Motor Skills saved successfully!', type: 'success' });
      } else {
        throw new Error('Failed to save');
      }
    } catch (error) {
      console.error('Error saving gross motor skills:', error);
      setToast({ message: 'Failed to save Gross Motor Skills. Please try again.', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="admin-dashboard min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Gross Motor Skills Assessment</h1>
        ...
        {selectedAge && currentSkills.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6">
              Gross Motor Skills for {selectedAge} months
            </h2>
        ...
        {!selectedAge && (
          <div className="bg-white rounded-xl shadow-lg p-8 sm:p-12 text-center">
            <p className="text-gray-500 text-base sm:text-lg italic">
              Please select child's age to view relevant gross motor skills
            </p>
          </div>
        )}
        ...
        {selectedAge && (
          <div className="flex justify-center pb-6 mt-6">
            ...
            >
              ...
              {saving ? 'Saving...' : 'Save Gross Motor Skills'}
            </button>
          </div>
        )}
      </div>
