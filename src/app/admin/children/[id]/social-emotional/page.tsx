...
export default function SocialEmotionalPage() {
  ...
  const loadAgeData = async (age: string) => {
    try {
      // Load existing social-emotional data for this specific age
      const response = await fetch(`/api/social-emotional?child_id=${childId}&age=${age}`);
      if (response.ok) {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(`Social-Emotional for ${selectedAge} months`, 15, yPos);
    yPos += 8;
    ...
    skills.forEach((skill, index) => {
      if (yPos > pageHeight - 40) {
        ...
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text('SOCIAL-EMOTIONAL ASSESSMENT (Continued)', pageWidth / 2, 20, { align: 'center' });
        yPos = 30;
        doc.setFontSize(9);
        doc.setFont('helvetica', 'normal');
      }
