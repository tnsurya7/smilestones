...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const event = events.find((e) => e.id === params.id && e.category === params.category);
...
export default function EventDetailPage({ params }: PageProps) {
  ...
  const categoryNames = {
    ...
    'teaching-institutes': 'Teaching Institutes',
  };

  const categoryName = categoryNames[event.category as keyof typeof categoryNames];
      ...
      <div className="pt-6 pb-4 bg-gray-50">
        <div className="container mx-auto px-4">
          <nav className="text-sm text-gray-600">
            ...
            <Link href={`/events/${event.category}`} className="hover:text-blue-600"> {categoryName}</Link> → 
